				...
				<div class="financial-inner-section ng-scope" ng-if="previewMode">
					<div ng-include="&#39;views/includes/financial/sheet/section-preview.html&#39;" class="ng-scope">
						<div class="grid-section">
							...
							@endif
							<div class="grid-col" style="white-space: nowrap;"><label>Estimate #</label><span> XXXX</span></div>
							<div class="grid-col"><label>Estimate Date </label><span> {{ \Carbon\Carbon::now(Settings::get('TIME_ZONE'))->format(config('jp.date_format'))}}</span></div>

							@if($company->present()->licenseNumber))
							<div class="grid-col"><label>Contractor license #</label><span> {{ $company->present()->licenseNumber) }}</span></div>
							@endif
						</div>
