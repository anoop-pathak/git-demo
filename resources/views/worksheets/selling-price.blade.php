			...
			<div class="jp-panel-heading">
				<h2>
					...
					<div class="pl-total" style="font-size:16px;padding-bottom:5px;padding-top:0;">
						<span>
							Selling Price <span class="help-text-selling-price">(Actual): </span>
							{{ currencyFormat($job->amount) }};</span>

							<span>
								Difference From Actual:
							</span>

							<?php $difference = $job->amount - $worksheet->total; ?>
							@if($difference >= 0) 
								<span class="ng-binding financial-profit">
								{{ currencyFormat($difference) }}
								</span>
							@else 
							<span class="ng-binding financial-loss">
								-{{ currencyFormat(abs($difference)) }}
							</span>
							@endif
				...
				<div id="financial-section-job">
					<div class="financial-inner-section ng-scope" ng-if="previewMode">
						<div ng-include="'views/includes/financial/sheet/section-preview.html'" class="ng-scope"><div>
						<table class="table table-striped table-hover table-full-width margin">
								<thead>
									<tr>
										<th style="width: 3%"><span>S.No</span></th>
										<th style="width: 13%"><span>Type</span></th>
										<th style="width: 10%"><span>Name</span></th>
										<th style="width: 34%"><span>Description</span></th>
										<th style="width: 10%"><span>Qty</span></th>
										<th style="width: 10%"><span>Unit</span></th>
										<th style="width: 10%"><span>Price/Qty</span></th>
										<th style="width: 10%"><span>Price($)</span></th>
									</tr>
								</thead>
								<tbody>
									...
									<tr>
										...
										<td>{{ $financialDetail->quantity }}</td>
										<td>{{ $financialDetail->unit }}</td>
										<td>{{ moneyFormat($financialDetail->unit_cost) }}</td>
										<?php
										$totalPrice  = $financialDetail->unit_cost * $financialDetail->quantity;
										?>
						...
						<div class="table-responsive preview-finance-tabel-container">
							<table style="float:left;" class="table table-striped table-hover table-full-width margin  preview-finance-tabel">
								<tbody>
									<tr>
										<td> Selling Price <span class="help-text-selling-price">(Actual)</span> </td>
										<td></td>
								...
								<table class="table table-striped table-hover table-full-width margin  preview-finance-tabel">
									<tbody><tr>
										<td> Selling Price <span class="help-text-selling-price">(Worksheet)</span> </td>
										<td>&nbsp;</td>
										<td class="ng-binding" style="text-align: right">{{ currencyFormat($worksheet->total) }}</td>
									</tr>
									<tr><th colspan="3	">&nbsp;</th></tr>
									<tr>
										<td> Difference From Actual </td>
										<td></td>
										<td style="text-align: right">
											<?php
											$difference = $job->amount - $worksheet->total;
											?>
											@if($difference >= 0)
												<span class="ng-binding financial-profit">
												{{ currencyFormat($difference) }}
