...
<body>
	...
	<?php	$createdDate = new Carbon\Carbon($material_list->created_at, Settings::get('TIME_ZONE')); ?>
	<div class="container">
				...
				<div class="financial-inner-section ng-scope" ng-if="previewMode">
					<div ng-include="&#39;views/includes/financial/sheet/section-preview.html&#39;" class="ng-scope">
						<div class="grid-section">
							...
							<?php
								$serialNumber = $material_list->serial_number;
								$label = "Material List #";
								$srs = \App\Models\Supplier::srs();
								$forSrs = ($srs && $srs->companySupplier && ($srs->id == $material_list->for_supplier_id));
								if($forSrs) {
									$label = "PO #";
								}
							?>

							<div class="grid-col" style="white-space: nowrap;"><label>{{ $label }}</label><span> {{ $serialNumber }}</span></div>
							<div class="grid-col"><label>Material List Date </label><span> {{$createdDate->format(config('jp.date_format'))}}</span></div>
							@if($deliveryDate = $job->material_delivery_date)
							<?php $deliveryDate = new Carbon\Carbon($deliveryDate, \Settings::get('TIME_ZONE')); ?>
							<div class="grid-col"><label>Delivery Date </label><span> {{$deliveryDate->format(config('jp.date_format'))}}</span></div>
							@endif
							@if(($purchaseOrderNumber = $job->purchase_order_number) && (!$forSrs))
							<div class="grid-col"><label>P.O. # </label><span>{{ $purchaseOrderNumber }}</span></div>
							@endif
						</div>

						@if(!$worksheet->hide_pricing)
							<div class="billed-box" style="float: right;">
								<div class="main-heading">
									<p style="">Material List Amount</p>

									<span>{{ showAmount($worksheet->calculateTotal()) }}</span>
								</div>
							</div>
						@endif
