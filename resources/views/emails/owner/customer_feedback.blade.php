			...
			<tbody>
				...
				<tr>
					<td cellspacing="0" cellpadding="0" style="color: rgb(0, 0, 0); background: #f9f9f9; padding: 20px 15px;">
						<p style="font-family: arial,sans-serif; margin: 0px; padding: 5px 2px;  font-size: 15px; color: rgb(106, 106, 106);">Dear {{ $owner->full_name }},</p>

						<?php
						$link = config('jp.site_job_url').$job->customer->id.'/job/'.$job->id.'/overview';
						?>
						...
						Hello!! You were just contacted by (<b><a href="{{ $link }}" target="_blank" style="color:#666; text-decoration:none">{{ $job->customer->full_name }} / {{ $job->number }}</a></b>) regarding the following:
						</p>
						@else
						<p style="font-family: arial,sans-serif; margin: 0px; padding: 5px 2px; font-size: 15px; color: rgb(106, 106, 106);">
							Congratulations!! You just received the below mentioned testimonial for <b><a href="{{ $link }}" target="_blank" style="color:#666; text-decoration:none">{{ $job->customer->full_name }} / {{ $job->number }}</a></b>.
						</p>
						@endif
						<table style="color:#000000;font-size:13px;font-family:arial,sans-serif;">
							<tr>
								<td style="vertical-align:top;padding:5px 0;width:150px;font-size: 15px; color: rgb(106, 106, 106);" cellspacing="0" cellpadding="0">Subject :</td><td style="vertical-align:top;padding:5px 0;font-size: 15px; color: rgb(106, 106, 106);"> {{ $input['subject'] }}</td>
							</tr>
							<tr>
								<td style="vertical-align:top;padding:5px 0;width:150px;font-size: 15px; color: rgb(106, 106, 106);" cellspacing="0" cellpadding="0">Description :</td><td style="vertical-align:top;padding:5px 0;font-size: 15px; color: rgb(106, 106, 106);"> {{ $input['description'] }}</td>
							</tr>
						</table>
					</td>
				</tr>
