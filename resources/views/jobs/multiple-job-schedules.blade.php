				...
				<table class="table">
					<thead>
						<tr>
							...
							<th style="width: 12%;">Job Rep / Estimator</th>
							<th style="width: 16%;">Work Crew</th>
							<th style="width: 7%;">Work Order(s)</th>
							<th style="width: 7%;">Material List(s)</th>
							<th style="width: 12%;">Trade Type</th>
							<th style="width: 10%;">Start Date</th>
							<th style="width: 10%;">End Date</th>
							<!-- <th style="width: 9%;">Off Date(s)</th> -->
							<th style="width: 10%;">#Schedule Days</th>
						</tr>
					</thead>
					<tbody>
						<?php $key = 0 ?>
						...
						<?php
							$job = $schedule->job;
							$jobIdReplace = $job->present()->jobIdReplaceWithLable;
							$jobIdReplace = (strlen($jobIdReplace) > 155) ? substr($jobIdReplace,0,155).'...' : $jobIdReplace;
									...
									<tbody>
										<tr>
											...
											<td style="width: 15%">
												...
												<p style="font-size: 13px">({{$secName}})</p>
												@endif
											</td>
											<td style="width: 12%">
											...
											<td style="width: 7%">
												{{ $schedule->workOrders->count() }}
											</td>
											<td style="width: 7%">
												{{ $schedule->materialLists->count() }}
											</td>
											<td style="width: 12%">
												{{ implode(', ', $schedule->trades->pluck('name')->toArray()) }}
											</td>
												...
												<td colspan="10">
													<div class="second-part">
														<div class="desc customer-desc">
															@if($job->name)
															<label class="job-heading">Job Name:</label>
															<span class="description">{{ $job->name }}</span>
															@endif
															@if($job->alt_id)
															<label class="job-heading">Job Number:</label>
															<span class="description">{{ $job->full_alt_id }}</span>
															@endif
															<label class="job-heading">Recurring:</label>
															<span class="description">{{ $schedule->present()->recurringText }}</span>
														</div>
														@if($job->duration_in_seconds)
														<div class="desc customer-desc">
															@if($job->isProject())
															<label style="float:left;" class="job-heading">Project Duration:</label>
															@else
															<label style="float:left;" class="job-heading">Job Duration:</label>
															@endif
															<span style="display:inline-block;padding-left:5px;width:50%;" class="text-justify">{!! $job->present()->jobDuration !!}
															</span>
														</div>
														@endif
														@if(( $job->address && $job->address->present()->fullAddressOneLine))
														<div class="desc customer-desc">
															<label class="job-heading">Job Address:</label>
															<span class="description">{!!lineBreak($job->address->present()->fullAddressOneLine) !!}</span>
														</div>
														@endif
														...
														<div class="desc customer-desc">
																<label class="job-heading">Work Crew Notes:</label>
