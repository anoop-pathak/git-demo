	...
	foreach($header as $key => $value){
		...
		switch ($key) {
			...
			case 'proposal_number':
				echo '<div class="grid-col" style="white-space: nowrap;"><label>Proposal # </label><span> XXXX</span></div>';
				break;
			case 'proposal_date':
				$dateTime = \Carbon\Carbon::now(Settings::get('TIME_ZONE'));
				echo '<div class="grid-col"><label>Proposal Date </label><span> '.$dateTime->format(config('jp.date_format')).'</span></div>';
				break;
			case 'job_contact':
