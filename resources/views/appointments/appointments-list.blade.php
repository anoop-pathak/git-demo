			...
			<div class="jobs-export">
				<div class="header-part">
					...
					@if(isset($filters['end_date']) && isset($filters['start_date']))
						<?php $startDate = new Carbon\Carbon($filters['start_date']); ?>
						@if(ine($filters, 'view') && ($filters['view'] === 'day'))
							{{ $startDate->format(config('jp.date_format')) }}
						@elseif(ine($filters, 'view') && ($filters['view'] === 'week'))
						<?php $endDate = new Carbon\Carbon($filters['end_date']);?>
						{{ $startDate->format(config('jp.date_format')) }} - {{ $endDate->format(config('jp.date_format')) }}
						@else
					...
					<div class="jobs-list">
						<div class="jobs-row">
							<div class="job-col job-detail-part">
								...
								<span class="address-info-sec">{{$appointment->location ?? ''}}</span>
								...
								<div><label>Appointment For:</label><p>
									{{ $appointment->present()->assignedUserName }}
								</p></div>
							...
							<div class="job-col job-detail-part">
								@if(!empty($customer = $appointment->customer))

									@if($customer->is_commercial)
										@if($secName = $customer->present()->secondaryFullName)
										<div>
											<label>Customer Name: </label>
											<p class="text-alignment" style="width: 200px;">
												{{ $secName }}
											</p>
										</div>
										@endif
						...
						<div class="desc">
							<p>
								<label class="job-heading">Appointment Note: </label>
