	...
	var dir = function ($injector, $timeout, $http, $filter, JPEvent) {
		return {
			...
			link: function (scope, iElement, iAttrs) {
				...
				var self = this;
				var AB_EDWARD = JPAPP.getConfig().AB_EDWARD_ENTERPRISES;
				...
				**/
				self.isBlair = function() {
					return ( parseInt(scope.company.id) == AB_EDWARD.COMPANY_ID );
				};
				...
				**/
				self.isTemplate2  = function(id, editMode, element) {
					...
					if( JP.haveValue(id) && AB_EDWARD.TEMPLATE_2.indexOf( JP.int(id) ) > -1) {
						return true
					}
				...
				self.isDropoDownTemplate = function(id, editMode, element) {
					...
					if( JP.haveValue(id) && (AB_EDWARD.TEMPLATE_ID.indexOf( JP.int(id)  ) > -1 )) {
						return true;
					}
