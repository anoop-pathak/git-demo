(function() {
	...
	var dir = function ($injector, $timeout, $http, $filter, JPEvent) {
		return {
			...
			link: function (scope, iElement, iAttrs) {
				...
				var self = this;
				var DANIEL_HOOD = JPAPP.getConfig().DANIEL_HOOD_ROOFING;
				...
				**/
				self.isBlair = function() {
					return ( parseInt(scope.company.id) == DANIEL_HOOD.COMPANY_ID );
				};
				...
				**/
				self.isTemplate2  = function(id, editMode, element) {
					...
					if( JP.haveValue(id) && DANIEL_HOOD.TEMPLATE_2.indexOf( JP.int(id) ) > -1) {
						return true
					}
				...
				self.isDropoDownTemplate = function(id, editMode, element) {
					...
					if( JP.haveValue(id) && (DANIEL_HOOD.TEMPLATE_ID.indexOf( JP.int(id)  ) > -1 )) {
						return true;
					}
	...
	*/
	angular
		.module('jobProgress')
		.directive('danielHoodRoofing', dir);
})();