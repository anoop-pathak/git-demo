(function() {
	...
	var dir = function ($injector, $timeout, $http, $filter, JPEvent) {
		return {
			...
			link: function (scope, iElement, iAttrs) {
				...
				var self = this;
				var FIRST_CLASS = JPAPP.getConfig().FIRST_CLASS_ROOFING;
				...
				**/
				self.isBlair = function() {
					return ( parseInt(scope.company.id) == FIRST_CLASS.COMPANY_ID );
				};
				...
				**/
				self.isTemplate2  = function(id, editMode, element) {
					...
					if( JP.haveValue(id) && FIRST_CLASS.TEMPLATE_2.indexOf( JP.int(id) ) > -1) {
						return true
					}
				...
				self.isDropoDownTemplate = function(id, editMode, element) {
					...
					if( JP.haveValue(id) && (FIRST_CLASS.TEMPLATE_ID.indexOf( JP.int(id)  ) > -1 )) {
						return true;
					}
	...
	*/
	angular
		.module('jobProgress')
		.directive('firstClassRoofing', dir);
})();