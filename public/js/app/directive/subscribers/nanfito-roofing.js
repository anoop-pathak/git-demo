	...
	var dir = function ($injector, $timeout, $http, $filter, JPEvent) {
		return {
			...
			link: function (scope, iElement, iAttrs) {
				...
				var self = this;
				var V_NANFITO_ROOFING = JPAPP.getConfig().V_NANFITO_ROOFING;
				...
				**/
				self.isBlair = function() {
					return ( parseInt(scope.company.id) == V_NANFITO_ROOFING.COMPANY_ID );
				};
				...
				**/
				self.isTemplate2  = function(id, editMode, element) {
					...
					if( JP.haveValue(id) && V_NANFITO_ROOFING.TEMPLATE_2.indexOf( JP.int(id) ) > -1) {
						return true
					}
				...
				self.isDropoDownTemplate = function(id, editMode, element) {
					...
					if( JP.haveValue(id) && (V_NANFITO_ROOFING.TEMPLATE_ID.indexOf( JP.int(id)  ) > -1 )) {
						return true;
					}
