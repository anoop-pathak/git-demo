	...
	var dir = function ($injector, $timeout, $http, $filter, JPEvent) {
		return {
			...
			link: function (scope, iElement, iAttrs) {
				...
				var self = this;
				var SOLAR_ME = JPAPP.getConfig().SOLAR_ME;
				...
				**/
				self.isBlair = function() {
					return ( parseInt(scope.company.id) == SOLAR_ME.COMPANY_ID );
				};
				...
				**/
				self.isTemplate2  = function(id, editMode, element) {
					...
					if( JP.haveValue(id) && SOLAR_ME.TEMPLATE_2.indexOf( JP.int(id) ) > -1) {
						return true
					}
				...
				self.isDropoDownTemplate = function(id, editMode, element) {
					...
					if( JP.haveValue(id) && (SOLAR_ME.TEMPLATE_ID.indexOf( JP.int(id)  ) > -1 )) {
						return true;
					}
