(function() {
	...
	var dir = function ($injector, $timeout, $http, $filter, JPEvent) {
		return {
			...
			link: function (scope, iElement, iAttrs) {
				...
				var self = this;
				var DOUBLE_D = JPAPP.getConfig().DOUBLE_D_CONTRACTORS;
				...
				**/
				self.isBlair = function() {
					return ( parseInt(scope.company.id) == DOUBLE_D.COMPANY_ID );
				};
				...
				**/
				self.isTemplate2  = function(id, editMode, element) {
					...
					if( JP.haveValue(id) && DOUBLE_D.TEMPLATE_2.indexOf( JP.int(id) ) > -1) {
						return true
					}
				...
				self.isDropoDownTemplate = function(id, editMode, element) {
					...
					if( JP.haveValue(id) && (DOUBLE_D.TEMPLATE_ID.indexOf( JP.int(id)  ) > -1 )) {
						return true;
					}
	...
	*/
	angular
		.module('jobProgress')
		.directive('doubledContractors', dir);
})();