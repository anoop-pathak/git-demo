...
class MoveJobResourceSettingsToCompanyFolderSettingCommand extends Command {
	 ...
	 * @var string
	 */
	protected $name = 'command:move_job_resource_settings_to_company_folder_settings';
	 ...
	 * @var string
	 */
	protected $description = 'Move Job Setting Resource into new table i.e company_resource_settings.';
		...
		$query = Company::with(['subscriber'])
			->whereNotIn('id', function($query) {
				$query->select('company_id')
					...
					->where('type', CompanyFolderSetting::JOB_FOLDERS);
			});
		...
		$query->chunk(100, function($companies) {
			foreach ($companies as $key => $company) {
				...
				try {
					...
					$jobFolders = $settingObj->get(CompanyFolderSetting::JOB_FOLDERS);

					foreach ($jobFolders as $key => $jobFolder) {
						$folder = CompanyFolderSetting::firstOrNew([
							'type' => CompanyFolderSetting::JOB_FOLDERS,
							'company_id' => $company->id,
							'name' => $jobFolder['name'],
						]);
						$folder->position = $key+1;
						$folder->locked = isTrue($jobFolder['locked']);
						$folder->save();
					}
