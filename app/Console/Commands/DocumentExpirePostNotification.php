...
class DocumentExpirePostNotification extends Command
{
     ...
     * @var string
     */
    protected $name = 'command:document_expire_post_notification';
     ...
     * @var string
     */
    protected $description = 'notify to admin that today document has been expired.';
    ...
    public function handle()
    {
        ...
        foreach ($documents as $key => $document) {
            $this->onExpiredNotification($document);
        }
    }

    /**
     * notification on document expired
     * @return [type] [description]
     */
    private function onExpiredNotification($document)
    {
        try {
            $this->sendNotification($document);
        } catch (\Exception $e) {
            $messageString = $e->getMessage() . ' in file ' . $e->getFile() . ' on line number ' . $e->getLine();
            Log::info('Document Expire pre notification: ' . $messageString);
        }
        return true;
    }
    ...
    private function sendNotification($document)
    {
        ...
        if ($job) {
            ...
            $emailContent = trans('response.events.email.document_expire_content.with_job.today', [
                'file_name' => $fileName,
                'job_redirect' => $jobRedirect
            ]);
        } else {
            $emailContent = trans('response.events.email.document_expire_content.without_job.today', [
                'file_name' => $fileName
            ]);
        }
        $messageString = trans('response.events.notifications.document_expired', ['file_name' => $fileName]);
    ...
    private function webNoticaction($messageString, $objectId, $recipients, $body)
    {
        try {
            ...
            $messageString = $e->getMessage() . ' in file ' . $e->getFile() . ' on line number ' . $e->getLine();
            Log::info('Document Expire pre web notification: ' . $messageString);
        }
    }
