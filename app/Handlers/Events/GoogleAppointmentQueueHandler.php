...
class GoogleAppointmentQueueHandler
{
	...
	public function insert($queueJob, $data)
	{
		try {
			if(!ine($data, 'user_id') 
				|| !ine($data, 'recurring_id')) return $queueJob->delete();
 			$appointment = $this->repo->getUserRecurringAppointment($data['recurring_id'], $data['user_id']);
			if(!$appointment)  return $queueJob->delete();
 			$user = $appointment->user;
			if(!$user) return $queueJob->delete();
 			$calendarClient = $user->googleCalendarClient;
			if(!$calendarClient) return $queueJob->delete();
 			if($appointment->user_id != $data['user_id']) return $queueJob->delete();
	...
	public function delete($queueJob, $data)
	{
		try {
 			if(!ine($data, 'user_id') 
				|| !ine($data, 'google_event_id')) return $queueJob->delete();
	...
	public function update($queueJob, $data)
	{
		try {
			...
			$previousUserId = $data['previous_user_id'] ?: null;
 			$appointment = $this->repo->getFirstRecurringAppointment($data['appointment_id'], true);
			if(!$appointment)  return $queueJob->delete();
			...
			if($calendarClient) {
 			...
 			if($appointment->jp_to_google_sync && ($appointment->user_id)) {
				...
				$this->manageOldRecurringEvents($user, 
					$appointment->id, 
					$calendarId, 
					$accessToken, 
					$oldGoogleEventId
	...
	{
		if(!($calendarId && $accessToken && $googleEventId)) return false;
 	...
 	private function updateGoogleEvent($appointment)
	{
		...
		if(!$appointment) return false;
 		if(!$appointment->jp_to_google_sync) return false;
 		$user = $appointment->user;
		...
		if(!$calendarClient) return false;        
 		if(($appointment->google_event_id)) {
			...
			$googleCalender = new GoogleCalenderServices;
			$googleCalender->update($calendarId, 
 	...
 	private function createGoogleEvent($appointment)
	{
		...
		if($appointment->google_event_id) {
 		...
 		if(!($eventExist)) {
			...
			$appointment->save();
			$googleCalender->insert($calendarId, 
				$accessToken, 
				$appointment, 
				$appointment->start_date_time, 
				$appointment->end_date_time, 
