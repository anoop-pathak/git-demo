...
use App\Events\JobDocumentDeleted;
...
class ResourcesRepository extends ScopedRepository
{
     ...
     * @return  bool
     */
    public function getDirWithName($name, $parentId){
        $dir = $this->make()->dir()->where(['name' => $name,'parent_id' => $parentId])->first();
     ...
     * @return bool
     */
    public function createDir($dirName, $parentDir, $locked = false, $name = null, $meta = [])
    {
        if (!$name) {
            $name = $dirName;
        }
     ...
     * @return bool
     */
    public function createFile($name, $parentDir, $mimeType, $size, $physicalName, $jobId = null, $meta = [])
    {
        try {
            $fileData = [
                'name' => $name,
                ...
                'external_full_url' => ine($meta, 'external_full_url') ? $meta['external_full_url'] : null
            ];
     ...
     * @return bool
     */
    public function getResources($id, $recursive, $filters = [])
    {

        $root = $this->make()->where('id', $id)->first();
        if (!$root) {
            return false;// if not root found return false..
        }
    ...
    {
        if (!ine($metaData, 'key') && !ine($metaData, 'value')) {
            return false;
        }
     ...
     * @return files
     */
    public function getSharedFiles($id, $filters = [])
    {
        $root = $this->make()
            ->where('id', $id)
            ->first();
     ...
     * @return $resource
     */
    public function createLink($parentDir, $name, $url, $type, $size = 0, $mimeType = null, $thumbUrl = null, $input = [])
    {
        $resource = $this->model;
    ...
    private function applyFilter($query, $filters)
    {
        ...
        if (ine($filters, 'mime_type') && $filters['mime_type'] == 'images') {
            $query->whereIn('mime_type', config('resources.image_types'));
        }

        if (ine($filters, 'mime_type') && $filters['mime_type'] == 'files') {
            $query->whereIn('mime_type', config('resources.docs_types'));
        }
