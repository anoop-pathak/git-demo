...
class EstimateUnderlaymentsController extends ApiController
{
	...
	public function index()
	{
		...
		try{
			$underLayments = $this->financialProductRepo->getUnderlayments($input);
			$limit = isset($input['limit']) ? $input['limit'] : config('jp.pagination_limit');

            if(!$limit) {
				$underLayments = $underLayments->get();
				$response = $this->response->collection($underLayments, new EstimateShinglesTransformer);
			} else {
				$underLayments = $underLayments->paginate($limit);
				$response =  $this->response->paginatedCollection($underLayments, new EstimateShinglesTransformer);
			}
	 ...
	 * @return Response
	 */
	public function markAsUnderlayments()
	{
		$input = Request::all();
        ...
        try {
			$this->financialProductRepo->markAsUnderlayements($financialProduct, $input['level_ids'], $input['conversion_size'], $input['manufacturer_id']);

            return ApiResponse::success([
				'message' => trans('response.success.saved', ['attribute' => 'Underlayment'])
			]);
		}catch(\Exception $e) {
