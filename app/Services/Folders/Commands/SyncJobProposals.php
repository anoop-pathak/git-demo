...
class SyncJobProposals extends BaseSync
{
    ...
    {
        return Proposal::groupBy('company_id')->pluck('company_id')->toArray();
    }

    /**
     * Sync jon proposals from the proposals table to folders table.
     *
     * @return boolean (true/false)
     */
    public function sync()
    {
        $type = Folder::JOB_PROPOSAL;
        $companyId = $this->companyId;

        $entityType = $type;
        $rootId = $this->getRootNodeId($type, $companyId);
        $items = $this->getProposalsByCompanyId($companyId, $type);
    ...
    /**
     * Get job proposal items on the basis of Company id which are not saved on folders table.
     *
     * @param Integer|Null $companyId: (optional) company id.
     * @param String $entityType: string of entity type.
     * @return Collection of Proposal items.
     */
    public function getProposalsByCompanyId($companyId = null, $entityType = null)
    {
        $tableName = (new Proposal)->getTable();
        $builder = Proposal::withTrashed()
     ...
     *
     * @param Proposal $item: Proposal model instance with data.
