...
class SyncJobWorkOrder extends BaseSync
{

    /**
     * Get company ids from the work orders table.
     *
     * @return array list of company ids.
     */
    public function getCompanyIds()
    {
        return MaterialList::where('type', MaterialList::WORK_ORDER)
                        ->groupBy('company_id')
                        ->pluck('company_id')
                        ->toArray();
    }
    ...
    {
        $type = Folder::JOB_WORK_ORDER;
        $companyId = $this->companyId;

        $entityType = $type;
        $rootId = $this->getRootNodeId($type, $companyId);
        $items = $this->getWorkOrdersByCompanyId($companyId, $type);
     ...
     */
    public function getWorkOrdersByCompanyId($companyId = null, $entityType)
    {
        ...
        $builder = MaterialList::withTrashed()
                        ...
                        ->where("$tableName.type", MaterialList::WORK_ORDER)
                        ->whereNull('f.id')
