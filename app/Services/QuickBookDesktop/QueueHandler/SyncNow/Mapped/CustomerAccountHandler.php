...
use Log;
use Queue;
use App\Services\QuickBookDesktop\QueueHandler\SyncNow\Mapped\JobAccountHandler;
	...
	*/
	public function fire($job, $meta)
	{
		...
		try {
			...
			Log::info('QBD Mapped Customer Account Handler Exception.');
			Log::info($e);
			if($groupId){
				$this->updateSyncCustomerStatus($companyId, $customerId, $groupId, QuickbookSyncCustomer::SYNC_FAILED, $qbCustomerId);
			}
	...
	private function createCustomerEntitiesTasks($customer, $qbCustomer, $meta)
	{
		...
		$metaData['object'] =  QuickBookDesktopTask::CUSTOMER;
		$metaData['object_id'] =  $qbCustomer->qb_id;
		$metaData['priority'] = QuickBookDesktopTask::PRIORITY_MAP_CUSTOMER;
		$metaData['origin'] = QuickBookDesktopTask::ORIGIN_QBD;
		TaskScheduler::addTask(QUICKBOOKS_IMPORT_CUSTOMER, $meta['user'], $metaData);
