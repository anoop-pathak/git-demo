...
use App\Services\QuickBookDesktop\QueueHandler\SyncNow\Mapped\ProjectAccountHandler;
use Log;
use Queue;
	...
	*/
	public function fire($queueJob, $meta)
	{
		...
		try {
			...
			if($jpJob && $qbJob){
				$this->createMappedJobEntitiesTasks($jpJob, $qbJob, $meta);
			}
			if($jpJob && !$qbJob){
				$this->createJpJobEntitiesTasks($jpJob, $meta);
			}

			if(!$jpJob && $qbJob){
				$this->createQbJobEntitiesTasks($qbJob, $meta);
			}

		} catch (Exception $e) {
			Log::info('QBD Mapped Job Account Handler Exception.');
			Log::info($e);
		}
	...
	private function createMappedJobEntitiesTasks($jpJob, $qbJob, $meta)
	{
		$metaData = $meta;
		$metaData['action'] = QuickBookDesktopTask::MAP;
		$metaData['object'] =  QuickBookDesktopTask::JOB;
		$metaData['object_id'] =  $qbJob->qb_id;
		$metaData['priority'] = QuickBookDesktopTask::PRIORITY_MAP_JOB;
		$metaData['origin'] = QuickBookDesktopTask::ORIGIN_QBD;

		TaskScheduler::addTask(QUICKBOOKS_IMPORT_JOB, $meta['user'], $metaData);
	...
	private function createJpJobEntitiesTasks($job, $meta)
	{
		$meta['action'] = QuickBookDesktopTask::CREATE;
		...
		if($meta['retain_financial'] == QuickbookSyncCustomer::RETAIN_QB_FINANCIAL){
			...
			$this->createJPFinancialsTask($job, $meta);
		}
	...
	private function createQbJobEntitiesTasks($job, $meta)
	{
		$meta['origin'] = QuickBookDesktopTask::ORIGIN_QBD;
		$meta['action'] = QuickBookDesktopTask::CREATE;
		$meta['object'] =  QuickBookDesktopTask::JOB;
		...
		if($meta['retain_financial'] == QuickbookSyncCustomer::RETAIN_JP_FINANCIAL){
			...
			$meta['origin'] = QuickBookDesktopTask::ORIGIN_QBD;
			$this->createDeleteFinancialTask($job->qb_id, $meta);
		} elseif(($meta['retain_financial'] == QuickbookSyncCustomer::RETAIN_QB_FINANCIAL)
			||($meta['retain_financial'] == QuickbookSyncCustomer::RETAIN_JP_AND_QB_FINANCIAL)
		){
			$this->createQBDFinancialsTask($job, $meta);
		}
	}
