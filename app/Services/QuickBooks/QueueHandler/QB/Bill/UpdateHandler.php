...
class UpdateHandler extends QBBaseTaskHandler
{
    ...
    function synch($task, $bill)
    {
        ...
        QBBill::createAttachmentTask($bill['entity'], $jpBill, QuickBookTask::UPDATE);
        return $jpBill;
    ...
    protected function checkPreConditions($bill){
        ...
        if(!$jpBill){
            $task = QBBill::createTask($bill->Id, QuickBookTask::CREATE, $this->task->created_source, QuickBookTask::ORIGIN_QB);
            $this->task->markFailed("Bill is not synced in JobProgress so sync it first.", $this->queueJob->attempts());
            return false;
        }
